...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_notification'])) {
    ...
    if ($targetType === 'agents') {
        $where = "role = 'agent' AND table_name = 'agents'";
    } elseif ($targetType === 'clients') {
        $where = "role = 'client' AND table_name = 'users'";
    }

    // Get users to notify
    $users = mysqli_query($conn, "SELECT id, table_name FROM users WHERE $where");
    ...
    while ($user = mysqli_fetch_assoc($users)) {
        ...
        mysqli_query($conn, "
            ...
            VALUES ('$userId', '$userTable', '$title', '$message', '$type', '$link', NOW())
        ");
    }
                                        ...
                                        <div class="d-flex align-items-center">
                                             <div class="flex-shrink-0">
                                                  <div class="bg-primary bg-opacity-10 p-3 rounded-circle">
                                                       <iconify-icon icon="solar:bell-outline" class="fs-24 text-primary"></iconify-icon>
                                                  </div>
                                             </div>
                                        ...
                                        <div class="d-flex align-items-center">
                                             <div class="flex-shrink-0">
                                                  <div class="bg-warning bg-opacity-10 p-3 rounded-circle">
                                                       <iconify-icon icon="solar:bell-minimalistic-outline" class="fs-24 text-warning"></iconify-icon>
                                                  </div>
                                             </div>
                                             <div class="flex-grow-1 ms-3">
                                        ...
                                        <div class="d-flex align-items-center">
                                             <div class="flex-shrink-0">
                                                  <div class="bg-info bg-opacity-10 p-3 rounded-circle">
                                                       <iconify-icon icon="solar:calendar-outline" class="fs-24 text-info"></iconify-icon>
                                                  </div>
                                             </div>
                                             <div class="flex-grow-1 ms-3">
                    ...
                    <div class="row">
                         ...
                         <div class="col-lg-8">
                              <div class="card">
                                   <div class="card-header d-flex justify-content-between align-items-center">
                                        ...
                                        <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#createNotifModal">
                                             <iconify-icon icon="solar:add-circle-outline"></iconify-icon> Send Notification
                                        </button>
                                   </div>
                                        ...
                                        <div class="list-group list-group-flush">
                                                  ...
                                                  <?php while ($notif = mysqli_fetch_assoc($getNotifications)): ?>
                                                       <div class="list-group-item notification-item <?= !$notif['is_read'] ? 'unread' : '' ?>">
                                                            <div class="d-flex align-items-start">
                                                                 <div class="flex-shrink-0">
                                                                      <div class="p-2 rounded bg-<?= getNotificationTypeColor($notif['type']) ?> bg-opacity-10 text-<?= getNotificationTypeColor($notif['type']) ?>">
                                                                           <iconify-icon icon="<?= getNotificationIcon($notif['type']) ?>" class="fs-20"></iconify-icon>
                                                                      </div>
                                                                 </div>
                                                                 <div class="flex-grow-1 ms-3">
                                                                      ...
                                                                      <a href="<?= htmlspecialchars($notif['link']) ?>" class="btn btn-sm btn-link ps-0">
                                                                           View <iconify-icon icon="solar:alt-arrow-right-outline"></iconify-icon>
                                                                      </a>
                                                                      <?php endif; ?>
                                                                 </div>
                                                                 <div class="flex-shrink-0">
                                                                      <div class="dropdown">
                                                                           ...
                                                                           <ul class="dropdown-menu">
                                                                                <li><a class="dropdown-item" href="?delete=<?= $notif['id'] ?>">Delete</a></li>
                                                                           </ul>
                                                                      </div>
                                                                 </div>
                                                            </div>
                                             ...
                                             <?php else: ?>
                                                  <div class="text-center py-5">
                                                       <iconify-icon icon="solar:bell-off-outline" class="fs-48 text-muted mb-2"></iconify-icon>
                         ...
                         <div class="col-lg-4">
                              <div class="card mb-3">
                                   ...
                                   <div class="card-body">
                                        <div class="d-grid gap-2">
                                             <button class="btn btn-outline-primary text-start" data-bs-toggle="modal" data-bs-target="#createNotifModal">
                                                  <iconify-icon icon="solar:send-outline"></iconify-icon> Send to All
                                             </button>
                                             <button class="btn btn-outline-info text-start" onclick="quickSend('agents')">
                                                  <iconify-icon icon="solar:users-group-rounded-outline"></iconify-icon> Notify Agents
                                             </button>
                                             <button class="btn btn-outline-success text-start" onclick="quickSend('clients')">
                                                  <iconify-icon icon="solar:user-plus-rounded"></iconify-icon> Notify Clients
                                             </button>
                                        </div>
                                   </div>
                              </div>
                              ...
                              <div class="card">
                                   ...
                                   <div class="card-body">
                                        <div class="list-group list-group-flush">
                                             <a href="#" class="list-group-item list-group-item-action" onclick="useTemplate('system_update')">
                                                  <iconify-icon icon="solar:refresh-circle-outline" class="text-primary"></iconify-icon>
                                                  System Update
                                             </a>
                                             <a href="#" class="list-group-item list-group-item-action" onclick="useTemplate('commission_update')">
                                                  <iconify-icon icon="solar:wallet-money-outline" class="text-success"></iconify-icon>
                                                  Commission Update
                                             </a>
                                             <a href="#" class="list-group-item list-group-item-action" onclick="useTemplate('new_feature')">
                                                  <iconify-icon icon="solar:star-outline" class="text-warning"></iconify-icon>
                                                  New Feature
                                             </a>
                                             <a href="#" class="list-group-item list-group-item-action" onclick="useTemplate('maintenance')">
                                                  <iconify-icon icon="solar:tool-outline" class="text-danger"></iconify-icon>
                                                  Maintenance Notice
                                             </a>
                                        </div>
                                   </div>
                    ...
                    <form method="POST">
                         ...
                         <div class="modal-body">
                              <div class="mb-3">
                                   ...
                                   <input type="text" name="title" class="form-control" required placeholder="e.g., System Update">
                              </div>
                              <div class="mb-3">
                              ...
                              <div class="mb-3">
                                   ...
                                   <input type="text" name="link" class="form-control" placeholder="e.g., /manager/reports">
                              </div>
                         </div>
