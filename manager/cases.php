...
// Handle Amount and Commission Update
if (isset($_POST['update_amounts']) && isset($_POST['case_id'])) {
    $caseId = intval($_POST['case_id']);
    $amount = floatval($_POST['amount']);
    $commissionAmount = floatval($_POST['commission_amount']);
    $commissionPaid = mysqli_real_escape_string($conn, $_POST['commission_paid']);

    $sql = "UPDATE `cases` SET `amount` = '$amount', `commission_amount` = '$commissionAmount', `commission_paid` = '$commissionPaid' WHERE `id` = $caseId";
    if (mysqli_query($conn, $sql)) {
        logActivity($manager['id'], 'admin', 'case_amounts_updated', 'case', $caseId, "Case amount updated to ₦$amount, commission to ₦$commissionAmount ($commissionPaid)");
        $msg = "Case amounts updated successfully";

        // If commission is marked as paid, create/update commission record
        if ($commissionPaid === 'paid' && $commissionAmount > 0) {
            $case = mysqli_fetch_assoc(mysqli_query($conn, "SELECT agent_id FROM cases WHERE id = $caseId"));
            if ($case && $case['agent_id']) {
                // Check if commission record exists
                $existingComm = mysqli_fetch_assoc(mysqli_query($conn, "SELECT id FROM commissions WHERE case_id = $caseId"));
                if ($existingComm) {
                    mysqli_query($conn, "UPDATE commissions SET amount = '$commissionAmount', status = 'paid', paid_date = NOW() WHERE case_id = $caseId");
                } else {
        ...
        $err = "Failed to update case amounts";
    }
}
        ...
        <div class="page-content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-flex justify-content-between align-items-center">
                            ...
                            <div>
                                <a href="cases.php?convert=new" class="btn btn-primary btn-sm" data-bs-toggle="modal"
                ...
                <?php if ($view): ?>
                    ...
                    <?php
                    $case = getCase($view);
                    $stageHistory = mysqli_query($conn, "SELECT h.*, a.fullname, u.email as admin_name
                                ...
                                <div class="card-body">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <p><?= htmlspecialchars($case['agent_name']) ?>
                                                (<?= htmlspecialchars($case['agent_code']) ?>)</p>
                                        </div>
                                    </div>
                                    <div class="row mb-3">
                                        <div class="col-md-4">
                                                            ...
                                                            <td>
                                                                <div class="btn-group btn-group-sm">
                                                                    <button class="btn btn-outline-success"
                                                                        onclick="verifyDoc(<?= $doc['id'] ?>)">Verify</button>
                                                                    <button class="btn btn-outline-danger"
                                                                        onclick="rejectDoc(<?= $doc['id'] ?>)">Reject</button>
                                                                </div>
                                                            </td>
                                                        </tr>
                                                    <?php endforeach; ?>
                            ...
                            <div class="card mt-3">
                                ...
                                <div class="card-body">
                                    <div class="d-grid gap-2">
                                        <a href="?action=on_hold&id=<?= $case['id'] ?>" class="btn btn-warning">Put On
                                            Hold</a>
                                        <a href="?action=completed&id=<?= $case['id'] ?>" class="btn btn-success">Mark
                                            Completed</a>
                                        <a href="?action=cancelled&id=<?= $case['id'] ?>" class="btn btn-danger">Cancel
                                            Case</a>
                                        <a href="commissions.php?case_id=<?= $case['id'] ?>" class="btn btn-info">View
                                            Commissions</a>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <form method="GET" class="row g-3">
                                        <div class="col-md-3">
                                            <select name="stage" class="form-select">
                                                ...
                                                <option value="offer" <?= $stageFilter == 'offer' ? 'selected' : '' ?>>Offer
                                                </option>
                                                <option value="visa" <?= $stageFilter == 'visa' ? 'selected' : '' ?>>Visa
                                                </option>
                                                <option value="completed" <?= $stageFilter == 'completed' ? 'selected' : '' ?>>
                                                    Completed</option>
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                            <select name="status" class="form-select">
                                                ...
                                                <option value="active" <?= $statusFilter == 'active' ? 'selected' : '' ?>>
                                                    Active</option>
                                                <option value="on_hold" <?= $statusFilter == 'on_hold' ? 'selected' : '' ?>>On
                                                    Hold</option>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="inquiry_id" class="form-select" required>
                                    ...
                                    while ($inq = mysqli_fetch_assoc($inquiries)):
                                        ?>
                                        <option value="<?= $inq['id'] ?>">
                                            <?= htmlspecialchars($inq['name']) ?> - <?= htmlspecialchars($inq['email']) ?>
