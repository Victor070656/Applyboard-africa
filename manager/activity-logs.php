...
$queryWhere = [];
if ($search) $queryWhere[] = "(description LIKE '%$search%' OR action LIKE '%$search%' OR ip_address LIKE '%$search%')";
if ($actionFilter) $queryWhere[] = "action = '$actionFilter'";
if ($userTypeFilter) $queryWhere[] = "user_type = '$userTypeFilter'";
if ($fromDate) $queryWhere[] = "DATE(created_at) >= '$fromDate'";
if ($toDate) $queryWhere[] = "DATE(created_at) <= '$toDate'";
$queryWhereClause = implode(' AND ', $queryWhere);
if ($queryWhereClause) $queryWhereClause = 'WHERE ' . $queryWhereClause;
...
<head>
     <meta charset="utf-8" />
     <title>ApplyBoard Africa Ltd || Activity Logs</title>
                                        ...
                                        <div class="d-flex align-items-center">
                                             <div class="flex-shrink-0">
                                                  <div class="bg-primary bg-opacity-10 p-3 rounded-circle">
                                                       <iconify-icon icon="solar:document-text-outline" class="fs-24 text-primary"></iconify-icon>
                                                  </div>
                                             </div>
                                        ...
                                        <div class="d-flex align-items-center">
                                             <div class="flex-shrink-0">
                                                  <div class="bg-success bg-opacity-10 p-3 rounded-circle">
                                                       <iconify-icon icon="solar:calendar-outline" class="fs-24 text-success"></iconify-icon>
                                                  </div>
                                             </div>
                                             <div class="flex-grow-1 ms-3">
                                        ...
                                        <div class="d-flex align-items-center">
                                             <div class="flex-shrink-0">
                                                  <div class="bg-info bg-opacity-10 p-3 rounded-circle">
                                                       <iconify-icon icon="solar:clock-circle-outline" class="fs-24 text-info"></iconify-icon>
                                                  </div>
                                             </div>
                                             <div class="flex-grow-1 ms-3">
                                                  <p class="text-muted mb-1">This Week</p>
                                                  <h4 class="mb-0"><?= number_format($weekLogs) ?></h4>
                                             </div>
                                        </div>
                                   </div>
                              </div>
                         ...
                         <div class="card-body">
                              <form method="GET" class="row g-3">
                                   <div class="col-md-3">
                                        ...
                                        <input type="text" name="search" class="form-control" placeholder="Description, action, or IP" value="<?= htmlspecialchars($search) ?>">
                                   </div>
                                   <div class="col-md-2">
                                   ...
                                   <div class="col-md-2">
                                        ...
                                        <select name="user_type" class="form-select">
                                             ...
                                             <option value="agent" <?= $userTypeFilter === 'agent' ? 'selected' : '' ?>>Agent</option>
                                             <option value="client" <?= $userTypeFilter === 'client' ? 'selected' : '' ?>>Client</option>
                                        </select>
                                   </div>
                                   <div class="col-md-2">
                                             ...
                                             <?php if (mysqli_num_rows($getLogs) > 0): ?>
                                                  <?php while ($log = mysqli_fetch_assoc($getLogs)): ?>
                                                       <tr>
                                                            ...
                                                            <td>
                                                                 <span class="badge <?= getUserTypeBadge($log['user_type']) ?>"><?= ucfirst($log['user_type']) ?></span>
                                                                 <div><?= htmlspecialchars($log['user_name'] ?? 'System') ?></div>
                                                            </td>
                                                            <td>
                                                                 <span class="badge <?= getActionBadge($log['action']) ?>"><?= ucfirst($log['action']) ?></span>
                                                            </td>
                                   ...
                                   <ul class="pagination justify-content-center mb-0">
                                        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                                             <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                                                  <a class="page-link" href="?page=<?= $i ?><?= $search ? '&search=' . urlencode($search) : '' ?><?= $actionFilter ? '&action=' . $actionFilter : '' ?><?= $userTypeFilter ? '&user_type=' . $userTypeFilter : '' ?><?= $fromDate ? '&from_date=' . $fromDate : '' ?><?= $toDate ? '&to_date=' . $toDate : '' ?>"><?= $i ?></a>
                                             </li>
                                        <?php endfor; ?>
                                   </ul>
                              </nav>
